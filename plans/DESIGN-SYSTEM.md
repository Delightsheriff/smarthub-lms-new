...
| Plans 001–024 (every remaining page — see `plans/000-index.md`) | ✅ Done. Verified against the actual diffs (not the completion report's prose) commit by commit, plus live browser testing of the highest-risk conversions (Cohort Workspace's 6 tabs, Referrals' table→Ledger, Jobs' IndexList) at 375px/desktop. Two prior passes (formerly plans 003/004, now deleted) had falsely claimed this same work "100% migrated" while only touching `PageHeader`; this pass is the real structural conversion. |
| Plan 025 (pluralize/StatusBadge rollout) | ✅ Done — 25 files' pluralize ternaries converted (verified zero remaining via grep), StatusBadge adopted at every real status-pill site found, icon-chip tints correctly left untouched. Registry gained `terminated` after verification found `WireInternship.status` could be a value not yet in it. |
